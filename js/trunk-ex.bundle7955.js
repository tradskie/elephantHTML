...
let BANKROLL = {
    ...
    "BSC": { 
        ...
        TRUNKBackedPool: "0xDb2c2741542E37bDa373bE49605cb8EFC5440455",
        Zero:"0x0000000000000000000000000000000000000000",
        TRUNKSecondaryPool:"0x612cE90180ec0185aD1aA09C74704a048a4ccCBA",
        TRUNKPerformanceBackedPool: "0x8655E8F5FeBEF300645d2CABAD84a1fDa72EEe3B",
...
var waiting = 0
let buyAmountInp, airdropAmountInp, sellAmountInp, transferAmountInp, buyEstimate, airdropEstimate, sellEstimate, transferEstimate, mintAmountInp, redeemAmountInp,sponsorAmountInp, sponsorEstimate, mintEstimate, redeemEstimate,
...
async function bindUI() {
    ...
    let mintTokens = async (e) => {
        try {
            ...
            amount = resolveInputAndBalance(amount, balance)
             ...
             if (amount.gt(web3js.utils.toBN(0))){
                mintEstimate.html(`${formatSun(amount)} BUSD ${approxStr} ${formatSun(amount * 0.99)} ${symbol}`)
             } else {
    ...
    mintAmountInp.on("change paste keyup", _.debounce(mintTokens, 250))
    ...
    let redeemTokens = async (e) => {
        try {
            ...
            if (amount.gt(web3js.utils.toBN(0))){
                let estimate = await elephantReserve.methods.estimateRedemptionV2(web3js.utils.toBN(amount + '')).call()
                console.log('amount-estimate', estimate)
                redeemEstimate.html(`${formatSun(amount)} TRUNK ${approxStr} ${formatSun(estimate[0])} BUSD + ${formatElephant(estimate[2])} ELEPHANT ${approxStr} ${formatSun(estimate[5])} BUSD`)
            } else {
...
async function showStats() {
    try {
        let web3js = getWeb3()
        let collateralizationRatio, collateralizationRatioLP
        let [totalTxs, players, tokenBalance, totalSupply, totalClaims, elephantTreasuryBalance, busdTreasuryBalance, lpBalance, stampStats] =
            await Promise.all(
                [
            ...
            let [liquidity, liquidity2,stampBalance] =
            await Promise.all(
                [
        ...
        collateralizationRatio = (busdTreasuryBalance > 0) ?  await elephantReserve.methods.collateralizationRatio().call() : 0
        collateralizationRatioLP = await elephantReserve.methods.collateralizationRatioLP().call()
        let elephantTreasuryBalanceUSD = (elephantTreasuryBalance > 0) ? (price * elephantTreasuryBalance) / 1e9 : 0
        let rewardBalance = await rewardPool.methods.creditBalance().call()
        let rewardAvailable = await rewardPool.methods.available().call()
        let secondaryAvailable = await secondaryPool.methods.available().call()
        let secondaryBalance = await secondaryPool.methods.creditBalance().call()
        let performanceAvailable = await performancePool.methods.available().call()
        let performanceBalance = await performancePool.methods.creditBalance().call() 

        let estimate = await elephantReserve.methods.estimateRedemption(web3js.utils.toBN('1000000000000000000')).call()
        let estimateSlippage = 100 * (1 - (estimate[5] / 0.99e18))
        $('#stamp-total-deposits').text(formatSunX(stampStats[1], 2))
        $('#partner-total-airdrops').text(formatSunX(stampStats[4], 2))
        $('#partner-total-partners').text(players)
        $('#stamp-total-payouts').text(formatSunX(stampStats[2], 2))
        $('#stampede-balance').text(`${formatSunX(stampBalance, 2)} TRUNK`)
        $('#stamp-totalTxs').text(numeral(stampStats[3]).format('0,0 a').toUpperCase())         
        $('#redemption-price').html(`${approxStr} ${formatSunX(estimate[5],6)}`) 
        $('#pancake-lp-balance').html(`${formatSun(lpBalance * 2)}`)
        $('#redemption-slippage').html(numeral(estimateSlippage).format('0.000') + ' %')      
        $('.secondarypool-balance').text(formatSun(secondaryBalance))
        $('.rewardpool-balance').text(formatSun(rewardBalance))
        $('.performance-balance').text(formatSun(performanceBalance))
        $('.performance-available').text(formatSun(performanceAvailable))
        $('#secondarypool-available').text(formatSun(secondaryAvailable))
        $('#rewardpool-available').text(formatSun(rewardAvailable))
        $('#collateral-ratio').text(numeral(collateralizationRatio / 1e6).format('0.000000'))
        $('#collateral-ratio-lp').html(`${approxStr} ${numeral(collateralizationRatioLP / 1e18).format('0.000000')}`)
...
async function showUserStats() {
    ...
    let [stampAvailable, stampStats, referrerAddress, teamSize ] = 
        await Promise.all(
            [
    ...
    if (totalSupply > 0 && balance > 0) {
        dailyEstimate = await rewardPool.methods.dailyEstimate(balance, totalSupply).call() 
        apr = dailyEstimate * 365 / balance      
    } else {
        dailyEstimate = await rewardPool.methods.dailyEstimate(totalSupply, totalSupply).call() 
    ...
    $('#partner-reward').text(formatSunX(stampStats[4], padding))
    $('#partner-total').text(teamSize)    

    $("#stamp-available").text(formatSunX(stampAvailable * 0.975, padding)) //2.5% fees
    ...
    $("#sponsor-pending").text(formatSunX(stampStats[3], padding))
    $("#sponsor-total").text(formatSunX(stampStats[4], padding))

           
    $('#staking-status').text(isStaking ? `Staking enabled` : `Staking disabled`)
    $('#stamp-staking-status').text(isStamp ? `Stampede enabled` : `Stampede disabled`)
...
async function sweepPools() {

    if ((await secondaryPool.methods.available().call() == 0)) {
        showAlert('LOW REWARDS!!!', 'Slow down there buddy, we need to accumulate more rewards before we sweep!')
        return
    }

    if (trunkDebug){
        secondaryPool.methods.sweep().estimateGas()
        .then((gasAmount) => console.log("mint gas amount", gasAmount))
    } else {
        secondaryPool.methods.sweep().send()
        .on('transactionHash', (tx) => refresh(tx))
        .on('error', (e) => txError(e))
    } 
...
async function stamp_buy() {
    ...
    */
    ...
    if (!isStaking) {
        showAlert('Enable Stampede', 'Stampede is not enabled.  Look for the toggle and make sure it is on (blue)!')
        return
    }
